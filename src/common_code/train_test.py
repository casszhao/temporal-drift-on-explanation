...
def train_hypersearch_model(model, training, development, loss_function, optimiser, seed,
    ...
    for epoch in range(epochs):
        ...
        for batch in training:
            ...
            assert batch["input_ids"].size(0) == len(
                    batch["labels"]), "Error: batch size for item 1 not in correct position"
        ...
        logging.info("*** epoch - {} | train loss - {} | dev f1 - {} | dev loss - {}".format(epoch + 1,
                                                                                             round(
                                                                                                 total_loss * training.batch_size / len(
                                                                                                     training), 2),
                                                                                             round(dev_results[
                                                                                                       "macro avg"][
                                                                                                       "f1-score"], 3),
